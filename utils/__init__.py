...
def _hf_no_weight_model_call(orig_func, klass, pretrained_model_name_or_path, *args, **kwargs):
    ...
    This is intentionally test-only and gated by ``HELM_SKIP_HF_MODEL_WEIGHTS``.
    ...
    model.eval()
    print(f"[utils/hf] skip model weights: {name_str} | {_hf_env_repr()}", flush=True)
...
def _hf_local_first_call(
    ...
    **kwargs,
    ...
    """
    if skip_model_weights and _env_flag_enabled("HELM_SKIP_HF_MODEL_WEIGHTS"):
    ...
    if os.path.isdir(name_str):
        print(f"[utils/hf] local path: {name_str} | {_hf_env_repr()}", flush=True)
        return orig_func(klass, pretrained_model_name_or_path, *args, **kwargs)
    ...
    try:
        ...
        print(f"[utils/hf] cache hit: {name_str} | {_hf_env_repr()}", flush=True)
        return result
    except Exception:
        print(
            f"[utils/hf] cache miss (will download): {name_str} | {_hf_env_repr()}",
            flush=True,
        )
        return orig_func(klass, pretrained_model_name_or_path, *args, **kwargs)
...
def _patch_hf_local_first() -> None:
    ...
    Triggered by HELM_HF_LOCAL_FIRST (set by conftest.py, survives uv run) or
    ...
    def _wrap(cls: type) -> None:
        ...
        if getattr(original, "_helm_hf_local_patched", False):
            return
        ...
        patched._helm_hf_local_patched = True  # type: ignore[attr-defined]
...
def _patch_mistral() -> None:
    ...
    Triggered by HELM_PATCH_MISTRAL (set by conftest.py, survives uv run) or
    ...
    try:
        ...
        if getattr(_orig, "_helm_patched", False):
            return
        ...
        _safe._helm_patched = True  # type: ignore[attr-defined]
        _cls._patch_mistral_regex = classmethod(_safe)
    except Exception:
        pass


if os.environ.get("PYTEST_CURRENT_TEST") or os.environ.get("HELM_HF_LOCAL_FIRST"):
    _patch_hf_local_first()

if os.environ.get("PYTEST_CURRENT_TEST") or os.environ.get("HELM_PATCH_MISTRAL"):
    _patch_mistral()
