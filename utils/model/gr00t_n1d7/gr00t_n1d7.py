...
from utils.configs.model.gr00t_n1d7 import Gr00tN1d7Config
from utils.model.modules.dit import AlternateVLDiT, DiT, SelfAttentionTransformer
from utils.model.modules.embodiment_conditioned_mlp import (
    CategorySpecificMLP,
...
class Gr00tN1d7ActionHead(nn.Module):
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        loss = action_loss.sum() / (mask_sum + 1e-6)

        if mask_sum <= 0 or not torch.isfinite(loss).all():
            def _tensor_stats(name: str, value: torch.Tensor) -> str:
                ...
                return (
                    ...
                    f"sum={detached.sum().item():.6g}, "
                    f"abs_sum={detached.abs().sum().item():.6g}, "
                    f"min={min_val:.6g}, max={max_val:.6g}"
                )

            raise FloatingPointError(
                "Invalid action loss detected.\n"
                f"loss={loss.detach()}, mask_sum={mask_sum.detach().float().item():.6g}\n"
                + "\n".join(
                    [
                        _tensor_stats("pred_actions", pred_actions),
                        _tensor_stats("velocity", velocity),
                        _tensor_stats("action_input.action", action_input.action),
                        _tensor_stats("action_mask", action_mask),
                        _tensor_stats("vl_embeds", vl_embeds),
                        _tensor_stats("state_features", state_features),
                    ]
                )
            )
...
def get_backbone_cls(config: Gr00tN1d7Config):
    if (
        ...
        from utils.model.modules.qwen3_backbone import Qwen3Backbone
...
class Gr00tN1d7(PreTrainedModel):
    ...
    supports_gradient_checkpointing = True
    # These modules are loaded from the HELM bundle's ``wam/`` component,
    # not from the root Transformers checkpoint. Tell Transformers not to
    # report their intentionally absent root keys as newly initialized.
    _keys_to_ignore_on_load_missing = [r"^wam_context_module\."]
    ...
    def __init__(
        ...
        from utils.model.wam_vla_lora import WamDreamZeroLoraContextModule

        if config.wam_model_path is None:
            raise ValueError("wam_model_path is required for HELM.")
        if config.wam_text_embedding_cache is None:
            raise ValueError("wam_text_embedding_cache is required for HELM.")
        self.wam_context_module = WamDreamZeroLoraContextModule(
            wam_model_path=config.wam_model_path,
            text_embedding_cache=config.wam_text_embedding_cache,
            state_dim=config.max_state_dim * config.state_history_length,
            num_context_tokens=config.wam_num_context_tokens or config.action_horizon,
            gr00t_backbone_dim=config.backbone_embedding_dim,
            freeze_vae=config.wam_freeze_vae,
            train_state_encoder=config.wam_train_state_encoder,
            train_context_tokens=config.wam_train_context_tokens,
            train_context_projector=config.wam_train_context_projector,
            lora_rank=config.wam_lora_rank,
            lora_alpha=config.wam_lora_alpha,
            lora_dropout=config.wam_lora_dropout,
            lora_target_modules=config.wam_lora_target_modules,
        )
    ...
    def _augment_backbone_with_wam_context(
        self,
        backbone_outputs: BatchFeature,
        action_inputs: BatchFeature,
        inputs: dict,
    ) -> BatchFeature:
        if self.wam_context_module is None:
            return backbone_outputs
        if "wam_images" not in inputs or "wam_text" not in inputs:
            raise KeyError(
                "WAM-VLA requires processor outputs 'wam_images' and 'wam_text'. "
                "Regenerate batches with the updated Gr00tN1d7Processor."
            )

        images = inputs["wam_images"]
        if torch.is_floating_point(images):
            images = images.to(self.device, dtype=self.dtype)
        else:
            images = images.to(self.device).to(self.dtype)

        state = action_inputs.state.view(action_inputs.state.shape[0], 1, -1)
        texts_raw = inputs["wam_text"]
        if isinstance(texts_raw, torch.Tensor):
            raise TypeError("wam_text must be a list/array of strings, not a tensor.")
        texts = [str(text) for text in list(texts_raw)]

        wam_context_tokens = self.wam_context_module(
            images=images,
            texts=texts,
            state=state,
        )
        wam_attention_mask = torch.ones(
            wam_context_tokens.shape[:2],
            device=backbone_outputs.backbone_attention_mask.device,
            dtype=backbone_outputs.backbone_attention_mask.dtype,
        )
        wam_image_mask = torch.zeros(
            wam_context_tokens.shape[:2],
        ...
        backbone_outputs["backbone_features"] = torch.cat(
            [backbone_outputs.backbone_features, wam_context_tokens],
            dim=1,
        )
        backbone_outputs["backbone_attention_mask"] = torch.cat(
            [backbone_outputs.backbone_attention_mask, wam_attention_mask],
            dim=1,
        )
        backbone_outputs["image_mask"] = torch.cat(
            [backbone_outputs.image_mask, wam_image_mask],
            dim=1,
        )
        return backbone_outputs
    ...
    def _forward_backbone(self, backbone_inputs: BatchFeature) -> BatchFeature:
        ...
        for key, value in backbone_outputs.items():
            if isinstance(value, torch.Tensor):
                backbone_outputs[key] = value.detach()
        return backbone_outputs
    ...
    def forward(self, inputs: dict) -> BatchFeature:
        ...
        backbone_outputs = self._augment_backbone_with_wam_context(
            backbone_outputs,
            action_inputs,
            inputs,
        )
