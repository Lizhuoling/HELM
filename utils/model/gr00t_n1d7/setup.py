...
from utils.configs.base_config import Config
from utils.configs.model.gr00t_n1d7 import Gr00tN1d7Config
from utils.data.dataset.factory import DatasetFactory
from utils.experiment.dist_utils import get_rank
from utils.model.base.model_pipeline import ModelPipeline
from utils.model.gr00t_n1d7.gr00t_n1d7 import Gr00tN1d7
from utils.model.gr00t_n1d7.processing_gr00t_n1d7 import Gr00tN1d7Processor
from utils.model.registry import register_model
...
def _initialize_missing_wam_context_tokens(model: Gr00tN1d7, missing_keys: list[str]) -> None:
    """Initialize the standalone WAM context-token parameter when absent from a checkpoint."""
    context_tokens_key = "wam_context_module.context_tokens"
    if context_tokens_key not in missing_keys:
        return

    wam_context_module = getattr(model, "wam_context_module", None)
    if wam_context_module is None:
        raise RuntimeError(f"Checkpoint reports missing {context_tokens_key}, but WAM-VLA is disabled.")

    context_tokens = wam_context_module.context_tokens
    if context_tokens.device.type == "meta":
        raise RuntimeError(f"Cannot initialize {context_tokens_key} while it is on the meta device.")

    with torch.no_grad():
        context_tokens.normal_(mean=0.0, std=0.02)

    if not torch.isfinite(context_tokens).all():
        raise RuntimeError(f"Initialized {context_tokens_key} contains non-finite values.")
    logging.info(
        "Initialized missing %s with N(0, 0.02): min=%g, max=%g, mean_abs=%g",
        context_tokens_key,
        context_tokens.detach().float().min().item(),
        context_tokens.detach().float().max().item(),
        context_tokens.detach().float().abs().mean().item(),
    )
    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            ...
            for key in (
                ...
                "use_wam_vla_lora",
                "wam_model_path",
                "wam_text_embedding_cache",
                "wam_num_context_tokens",
                "wam_freeze_vae",
                "wam_train_state_encoder",
                "wam_train_context_tokens",
                "wam_train_context_projector",
                "wam_lora_rank",
                "wam_lora_alpha",
                "wam_lora_dropout",
                "wam_lora_target_modules",
                "backbone_trainable_params_fp32",
                "use_relative_action",
                "load_bf16",
            ):
                setattr(model_config, key, getattr(self.config.model, key))
            ...
            _initialize_missing_wam_context_tokens(model, missing_keys)

            unexpected_keys = loading_info.get("unexpected_keys", [])
            mismatched_keys = loading_info.get("mismatched_keys", [])
            # WAM is instantiated from its own bundle component. Its parameters
            # are intentionally absent from the root model checkpoint, so never
            # report them as root-checkpoint errors regardless of source config.
            wam_missing = [k for k in missing_keys if k.startswith("wam_context_module.")]
            if wam_missing:
                logging.info("Initialized %d WAM parameters from bundle component", len(wam_missing))
            other_missing = [
                k for k in missing_keys
                if "mask_token" not in k and not k.startswith("wam_context_module.")
            ]
            unexpected_keys = [k for k in unexpected_keys if not k.startswith("wam_context_module.")]
            mismatched_keys = [k for k in mismatched_keys if not str(k).startswith("wam_context_module.")]
            errors = []
            if other_missing:
            ...
            logging.info(
                "Verified HELM root checkpoint: required_missing=0, unexpected=0, "
                "mismatched=0; WAM weights verified separately from the bundle"
            )
        ...
        if self.config.model.model_dtype == "bfloat16":
            logging.info("Casting HELM model parameters and buffers to bfloat16")
