...
from utils.configs.data.embodiment_configs import ModalityConfig
from utils.data.embodiment_tags import EmbodimentTag
from utils.data.interfaces import BaseProcessor
from utils.data.state_action.state_action_processor import StateActionProcessor
from utils.data.utils import parse_modality_configs, to_json_serializable
    ...
    def __call__(self, features: list[Dict[str, Any]]) -> BatchFeature:
        ...
        for key in keys:
            ...
            if key == "vlm_content":
                ...
                batch[key] = [str(v) for v in values]
            elif key == "wam_images":
...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def process_observation(self, observation: dict[str, Any], embodiment_tag: EmbodimentTag):
        ...
        raw_language = [str(lang) for lang in observation[language_key]]
        language = [
            re.sub(r"[^\w\s]", "", lang.lower()) if self.formalize_language else lang
            for lang in raw_language
        ]
        ...
        transformed_observation["wam_images"] = torch.from_numpy(transformed_images[:, :1])
        transformed_observation["wam_text"] = np.array(raw_language, dtype=object)
    ...
    def _get_vlm_inputs(
        ...
        vlm_inputs["wam_images"] = torch.from_numpy(stacked_images[:1].copy())
        vlm_inputs["wam_text"] = wam_language if wam_language is not None else language
        return vlm_inputs
