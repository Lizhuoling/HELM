...
class FinetuneConfig:
    ...
    pretrain_path: str
    """Path to the self-contained HELM pretraining bundle."""
    ...
    If None, use the pre-registered modality config in `utils/configs/data/embodiment_configs.py`.
    """
    ...
    """If True, load the VLM backbone parameters in bfloat16."""
    ...
    tune_vlln: bool = True
    """If True, fine-tune the VLM feature normalization/self-attention adapters."""

    wam_num_context_tokens: int | None = None
    """Number of WAM context tokens. Defaults to GR00T action_horizon when unset."""

    wam_freeze_vae: bool = True
    """If True, freeze the DreamZero/Wan VAE."""

    wam_train_state_encoder: bool = True
    """If True, train the WAM state/status encoder."""

    wam_train_context_tokens: bool = True
    """If True, train the learned WAM context-token parameters."""

    wam_train_context_projector: bool = True
    """If True, train the WAM-to-GR00T feature projection."""

    wam_lora_rank: int = 4
    """Rank of each DreamZero LoRA adapter."""

    wam_lora_alpha: int = 4
    """Scaling alpha for each DreamZero LoRA adapter."""

    wam_lora_dropout: float = 0.0
    """Dropout applied on DreamZero LoRA adapter inputs."""

    wam_lora_target_modules: str = (
        r"blocks\.\d+\."
        r"(self_attn\.(q|k|v|o)|cross_attn\.(q|k|v|o)|ffn\.(0|2))"
    )
    """Regular expression selecting DreamZero block projections adapted with LoRA."""
    ...
    wandb_project: str = "finetune-helm"
    ...
    task_episode_ratio: float | None = None
    """Ratio of the first episodes retained independently for each task."""
    ...
    auto_resume_from_output_dir: bool = True
    """If True, resume trainer state from the latest checkpoint in output_dir when present."""

    skip_weight_loading: bool = False
    """If True, skip loading model weights from pretrain_path (architecture only).
    The processor (tokenizer/config) is still loaded from pretrain_path.
