...
class Gr00tTrainer(Trainer):
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if self.state.max_steps is not None and self.state.max_steps > 0:
            logs = {
    ...
    def compute_loss(
        ...
        *and* model outputs, we calculate accuracy and push it to the logger.
        ...
        if not torch.isfinite(loss).all():
            diagnostics = [f"Non-finite training loss detected: {loss.detach()}"]
            for key in ("action", "action_mask", "state"):
                value = inputs.get(key)
                if isinstance(value, torch.Tensor):
                    finite = torch.isfinite(value) if torch.is_floating_point(value) else None
                    diagnostics.append(
                        f"{key}: shape={tuple(value.shape)}, dtype={value.dtype}, "
                        f"sum={value.detach().float().sum().item():.6g}, "
                        f"abs_sum={value.detach().float().abs().sum().item():.6g}, "
                        f"min={value.detach().float().min().item():.6g}, "
                        f"max={value.detach().float().max().item():.6g}"
                    )
                    if finite is not None:
                        diagnostics.append(
                            f"{key}: finite={finite.all().item()}, "
                            f"nan_count={torch.isnan(value).sum().item()}, "
                            f"inf_count={torch.isinf(value).sum().item()}"
                        )
