...
from utils.experiment.trainer import Gr00tTrainer, ProfCallback
from utils.experiment.utils import BestMetricCheckpointCallback, CheckpointFormatCallback
from utils.model import MODEL_REGISTRY
from utils.initial_actions import INITIAL_ACTIONS_FILENAME, save_initial_actions
...
def run(config: Config):
    ...
    if config.training.use_wandb and global_rank == 0:
        ...
        config_dict = {
            **config.__dict__,
            "git_commit_hash": os.environ.get("HELM_COMMIT_HASH", "unknown"),
        }
    ...
    resume_from_checkpoint = True if config.training.auto_resume_from_output_dir else None
