...
- HelmPolicyBase: Base policy class for model inference
- HelmSimPolicyWrapper: Wrapper for compatibility with existing Gr00t simulation environments
"""
...
from utils.data.embodiment_tags import FINETUNE_ONLY_TAGS, POSTTRAIN_TAGS, EmbodimentTag
from utils.data.interfaces import BaseProcessor
from utils.data.language_utils import normalize_language_text
from utils.data.types import MessageType, ModalityConfig, VLAStepData
...
class HelmPolicyBase(BasePolicy):
    ...
    def __init__(
        ...
        *,
        ...
        wam_model_path: str | None = None,
        wam_text_embedding_cache: str | None = None,
    ):
        ...
        Args:
            ...
            pretrain_path: Path to the self-contained HELM pretraining bundle.
            vlm_model_path: Optional local path or Hugging Face ID for the VLM backbone.
            wam_model_path: Optional internal component path; prefer pretrain_path.
            wam_text_embedding_cache: Optional internal cache path; prefer pretrain_path.
        """
        ...
        if pretrain_path is not None:
            ...
            bundle = resolve_pretrain_bundle(pretrain_path)
            vlm_model_path = str(bundle["vlm"])
            wam_model_path = str(bundle["wam"])
            wam_text_embedding_cache = str(bundle["text_embeddings"])

        # Load the pretrained model and move to target device with bfloat16 precision
        model_kwargs = {}
        if vlm_model_path or wam_model_path or wam_text_embedding_cache:
            model_config = AutoConfig.from_pretrained(model_dir)
            if vlm_model_path:
                model_config.model_name = vlm_model_path
                model_config.load_bf16 = True
            if wam_model_path:
                model_config.wam_model_path = wam_model_path
            if wam_text_embedding_cache:
                model_config.wam_text_embedding_cache = wam_text_embedding_cache
        ...
        processor_kwargs = {"model_name": vlm_model_path} if vlm_model_path else {}
        ...
        # HELM uses exactly one language field for each supported embodiment.
...
class HelmSimPolicyWrapper(PolicyWrapper):
    """Adapt HELM control signals to the RoboCasa GR1 simulation interface.
    ...
    'state.joint_positions') and the nested format expected by HelmPolicyBase.

    **Important**: If you are using other environments, custom robots, or building new environments,
    you should use `HelmPolicyBase` directly and format your observations according to its interface.
    ...
    def __init__(self, policy: HelmPolicyBase, *, strict: bool = True):
        """Initialize the wrapper around a HelmPolicyBase instance.

        Args:
            policy: The HelmPolicyBase instance to wrap
            strict: Whether to enforce strict validation (default: True)
        """
        super().__init__(policy, strict=strict)
        self.policy: HelmPolicyBase = policy
        assert len(self.policy.modality_configs["language"].delta_indices) == 1, (
            "Only one language delta index is supported"
        )

    @staticmethod
    def _resolve_sim_language_key(observation: dict[str, Any], expected_key: str) -> str:
        if expected_key in observation:
            return expected_key
        for candidate in (
    ...
    def check_observation(self, observation: dict[str, Any]) -> None:
        ...
        Unlike HelmPolicyBase.check_observation which expects nested dicts, this expects flat keys.
        ...
        for language_key in modality_configs["language"].modality_keys:
            language_key = self._resolve_sim_language_key(observation, language_key)
    ...
    def _get_action(
        ...
        into the nested format expected by HelmPolicyBase, computes actions, and transforms
        ...
        """
        # Transform flat observation format to nested format expected by HelmPolicyBase
        new_obs = {}
        for modality in ["video", "state", "language"]:
            new_obs[modality] = {}
            for key in self.policy.modality_configs[modality].modality_keys:
                if modality == "language":
                    parsed_key = self._resolve_sim_language_key(observation, key)
                else:
                ...
                if modality == "language":
                    ...
                    new_obs[modality][key] = [[normalize_language_text(item)] for item in arr]
                    if not getattr(self, "_logged_language_once", False):
                        print(
                            f"[eval language] source_key={parsed_key} "
                            f"model_key={key} text={new_obs[modality][key][0][0]!r}",
                            flush=True,
                        )
        ...
        # Compute actions using the underlying HelmPolicyBase
    ...
    def check_action(self, action: dict[str, Any]) -> None:
        ...
        Unlike HelmPolicyBase.check_action which expects nested dicts, this expects flat keys.
