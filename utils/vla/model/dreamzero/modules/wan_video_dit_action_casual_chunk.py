...
from utils.vla.model.dreamzero.modules.wan2_1_attention import AttentionModule
from utils.vla.model.dreamzero.modules.action_encoding import (
...
class CausalWanSelfAttention(nn.Module):
    ...
    @staticmethod
    def _apply_1d_action_rope(x: torch.Tensor, freqs_action: torch.Tensor) -> torch.Tensor:
        """Apply 1D action RoPE to an action-token sequence."""
        B, seq_len, n, D = x.shape
        freqs_action = freqs_action[:seq_len]

        if ENABLE_TENSORRT:
            freqs = freqs_action.unsqueeze(0).unsqueeze(2)
        ...
        freqs = freqs_action.view(seq_len, 1, -1).unsqueeze(0)
        return torch.view_as_real(x_complex * freqs).flatten(3).type_as(x)

    def _apply_video_action_rope(
        self,
        x: torch.Tensor,
        freqs: torch.Tensor,
        freqs_action: torch.Tensor,
        video_len: int,
        noisy_action_len: int,
        clean_action_cond_len: int,
    ) -> torch.Tensor:
        """Apply video RoPE to video tokens and action RoPE to both action streams."""
        assert x.shape[1] == video_len + noisy_action_len + clean_action_cond_len
        ...
        if video_len > 0:
            pieces.append(
                rope_action_apply(
                    x=x[:, :video_len],
                    freqs=freqs[:video_len],
                    freqs_action=freqs_action,
                    freqs_state=freqs_action,
                    action_register_length=None,
                )
            )

        noisy_start = video_len
        noisy_end = noisy_start + noisy_action_len
        if noisy_action_len > 0:
            pieces.append(
                self._apply_1d_action_rope(
                    x[:, noisy_start:noisy_end],
                    freqs_action[:noisy_action_len],
                )
            )

        if clean_action_cond_len > 0:
            pieces.append(
                self._apply_1d_action_rope(
                    x[:, noisy_end:],
                    freqs_action[:clean_action_cond_len],
                )
            )
    ...
    def _process_decoupled_noisy_image_blocks(
        self,
        noisy_image_q,
        noisy_image_k,
        noisy_image_v,
        ...
        """Noisy video attends to clean image context, itself, and clean action condition."""
        block_size = self.frame_seqlen * self.num_frame_per_block
        num_blocks = (half_frames - 1) // self.num_frame_per_block
        output = torch.empty_like(noisy_image_q)
        ...
        for block_idx in range(num_blocks):
            ...
            k_context = torch.cat(
                [
                    clean_image_k[:, :clean_end],
                    noisy_image_k[:, noisy_start:noisy_end],
                    clean_action_cond_k[:, action_start:action_end],
                ],
                dim=1,
            )
            v_context = torch.cat(
                [
                    clean_image_v[:, :clean_end],
                    noisy_image_v[:, noisy_start:noisy_end],
                    clean_action_cond_v[:, action_start:action_end],
                ],
                dim=1,
            )
            output[:, noisy_start:noisy_end] = self.attn(q_block, k_context, v_context)
    ...
    def _process_decoupled_noisy_action_blocks(
        self,
        noisy_action_q,
        noisy_action_k,
        noisy_action_v,
        ...
        """Noisy action targets attend to past clean actions, but not current/future actions."""
        num_blocks = (half_frames - 1) // self.num_frame_per_block
        if num_blocks == 0:
            return torch.empty_like(noisy_action_q)
        ...
        for block_idx in range(num_blocks):
            ...
            k_context = torch.cat(
                [
                    clean_image_k[:, :clean_end],
                    clean_action_cond_k[:, :action_start],
                    noisy_action_k[:, action_start:action_end],
                ],
                dim=1,
            )
            v_context = torch.cat(
                [
                    clean_image_v[:, :clean_end],
                    clean_action_cond_v[:, :action_start],
                    noisy_action_v[:, action_start:action_end],
                ],
                dim=1,
            )
    ...
    def _process_clean_action_cond_blocks(
        self,
        clean_action_cond_q,
        clean_action_cond_k,
        clean_action_cond_v,
        ...
        """Update clean action-condition tokens without leaking future chunks."""
        if clean_action_cond_q.shape[1] == 0:
            return clean_action_cond_q

        assert action_horizon == clean_action_cond_q.shape[1]
        assert action_horizon % self.num_action_per_block == 0

        output = torch.empty_like(clean_action_cond_q)
        num_blocks = action_horizon // self.num_action_per_block
        for block_idx in range(num_blocks):
            action_start = block_idx * self.num_action_per_block
            action_end = action_start + self.num_action_per_block
            q_block = clean_action_cond_q[:, action_start:action_end]
            output[:, action_start:action_end] = self.attn(
                q_block,
                clean_action_cond_k[:, :action_end],
                clean_action_cond_v[:, :action_end],
            )
    ...
    def forward(
        ...
        freqs_state: torch.Tensor,
        action_register_length: int | None,
        clean_action_cond_length: int = 0,
        ...
        if kv_cache is None:
            if is_tf:
                ...
                if clean_action_cond_length:
                    noisy_video_len = half_seq_len
                    noisy_action_len = action_register_length - clean_action_cond_length
                    rq_noisy = self._apply_video_action_rope(
                        q_noisy,
                        freqs=freqs,
                        freqs_action=freqs_action,
                        video_len=noisy_video_len,
                        noisy_action_len=noisy_action_len,
                        clean_action_cond_len=clean_action_cond_length,
                    )
                    rk_noisy = self._apply_video_action_rope(
                        k_noisy,
                        freqs=freqs,
                        freqs_action=freqs_action,
                        video_len=noisy_video_len,
                ...
                if action_register_length is not None and clean_action_cond_length:
                    # Teacher-forcing decoupled structure:
                    # Clean half: [image tokens only]
                    # Noisy half: [image tokens][noisy action targets][clean action condition]
                    ...
                    assert clean_action_cond_length == action_horizon
                    assert action_register_length == action_horizon + clean_action_cond_length
                    assert roped_query.shape[1] == half_seq_len + noisy_image_seq_len + action_horizon + clean_action_cond_length
                    ...
                    clean_action_cond_start = noisy_action_start + action_horizon

                    noisy_image_q = roped_query[:, noisy_image_start:noisy_action_start]
                    noisy_action_q = roped_query[:, noisy_action_start:clean_action_cond_start]
                    clean_action_cond_q = roped_query[:, clean_action_cond_start:]

                    noisy_image_k = roped_key[:, noisy_image_start:noisy_action_start]
                    noisy_action_k = roped_key[:, noisy_action_start:clean_action_cond_start]
                    clean_action_cond_k = roped_key[:, clean_action_cond_start:]

                    noisy_image_v = v[:, noisy_image_start:noisy_action_start]
                    noisy_action_v = v[:, noisy_action_start:clean_action_cond_start]
                    clean_action_cond_v = v[:, clean_action_cond_start:]

                    clean_image_outputs = self._process_clean_image_only(
                        clean_image_q, clean_image_k, clean_image_v, clean_frames)
                    clean_action_cond_outputs = self._process_clean_action_cond_blocks(
                        clean_action_cond_q, clean_action_cond_k, clean_action_cond_v,
                        action_horizon)
                    noisy_image_outputs = self._process_decoupled_noisy_image_blocks(
                        noisy_image_q, noisy_image_k, noisy_image_v,
                        clean_image_k, clean_image_v,
                        clean_action_cond_k, clean_action_cond_v,
                        noisy_frames, action_horizon)
                    noisy_action_outputs = self._process_decoupled_noisy_action_blocks(
                    ...
                    x = torch.cat([
                        ...
                        clean_action_cond_outputs,
                    ], dim=1)

                elif action_register_length is not None:
            ...
            if action_register_length is not None and clean_action_cond_length:
                noisy_action_len = action_register_length - clean_action_cond_length
                video_len = s - action_register_length
                ...
                roped_query = torch.cat(
                    [
                        ...
                        self._apply_1d_action_rope(
                            q[:, video_len:video_len + noisy_action_len],
                            freqs_action[action_offset:action_offset + noisy_action_len],
                        ),
                        self._apply_1d_action_rope(
                            q[:, video_len + noisy_action_len:],
                            freqs_action[action_offset:action_offset + clean_action_cond_length],
                        ),
                    ],
                    dim=1,
                ).type_as(v)
                roped_key = torch.cat(
                    [
                        ...
                        self._apply_1d_action_rope(
                            k[:, video_len:video_len + noisy_action_len],
                            freqs_action[action_offset:action_offset + noisy_action_len],
                        ),
                        self._apply_1d_action_rope(
                            k[:, video_len + noisy_action_len:],
                            freqs_action[action_offset:action_offset + clean_action_cond_length],
                        ),
                    ],
                    dim=1,
            ...
            roped_clean_action_query: torch.Tensor | None = None
            roped_clean_action_key: torch.Tensor | None = None
            clean_action_v: torch.Tensor | None = None

            if action_register_length is not None:
                noisy_action_len = action_register_length - clean_action_cond_length
                roped_action_query = roped_query[:, -action_register_length:-clean_action_cond_length] if clean_action_cond_length else roped_query[:, -action_register_length:]
                roped_action_key = roped_key[:, -action_register_length:-clean_action_cond_length] if clean_action_cond_length else roped_key[:, -action_register_length:]
                action_v = v[:, -action_register_length:-clean_action_cond_length] if clean_action_cond_length else v[:, -action_register_length:]
                if clean_action_cond_length:
                    roped_clean_action_query = roped_query[:, -clean_action_cond_length:]
                    roped_clean_action_key = roped_key[:, -clean_action_cond_length:]
                    clean_action_v = v[:, -clean_action_cond_length:]
            ...
            if action_register_length is not None and clean_action_cond_length:
                assert roped_clean_action_query is not None
                assert roped_clean_action_key is not None
                assert clean_action_v is not None
                video_x = self.attn(
                    roped_query,
                    torch.cat([new_k, roped_clean_action_key], dim=1),
                    torch.cat([new_v, clean_action_v], dim=1),
                )
                action_x = self.attn(
                    roped_action_query,
                ...
                clean_action_x = self.attn(
                    roped_clean_action_query,
                    roped_clean_action_key,
                    clean_action_v,
                )
                x = torch.cat([video_x, action_x, clean_action_x], dim=1)
            elif action_register_length is not None:
                x = self.attn(
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        y, updated_kv_cache = self.self_attn(
            ...
            freqs_state=freqs_state,
            action_register_length=action_register_length,
            clean_action_cond_length=clean_action_cond_length,
            kv_cache=kv_cache,
            is_tf=is_tf,
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_blocks(
        ...
        clean_action_cond: torch.Tensor | None,
        ...
        if action is not None:
            # Intentional for current VITRA training: only one human embodiment is used.
            ...
            if clean_action_cond is not None:
                assert state is None, "State tokens are omitted when clean_action_cond is used."
                cond_timestep = torch.zeros_like(timestep_action)
                clean_action_cond_features = self.action_encoder(
                    clean_action_cond, cond_timestep, embodiment_id
                )
                clean_action_cond_length = clean_action_cond_features.shape[1]
                assert clean_action_cond_length == action_length
                state_features = None
                action_register = torch.cat([action_features, clean_action_cond_features], dim=1)
            else:
                state_features = self.state_encoder(state, embodiment_id)
                clean_action_cond_length = 0
        ...
        if action is not None:
            ...
            if clean_action_cond is not None:
                cond_timestep = torch.zeros_like(timestep_action)
                timestep = torch.cat([timestep, timestep_action, cond_timestep], dim=1)
            else:
        ...
        # context_lens for T2V/TI2V cross attention
        if self.model_type in ('t2v', 'ti2v'):
            context_lens = torch.tensor([context.shape[1]] * B, dtype=torch.long, device=context.device)
        else:
            context_lens = None
    ...
    def _forward_inference(
        ...
        x_video, action_noise_pred, updated_kv_caches = self._forward_blocks(
            ...
            timestep_action=timestep_action,
            state=state,
            clean_action_cond=clean_action_cond,
            kv_cache=kv_cache,
            current_start_frame=current_start_frame,
        )
    ...
    def _forward_train(
        ...
        # context_lens: needed by WanT2VCrossAttention (Wan2.2 TI2V), not by I2V
        if self.model_type in ('t2v', 'ti2v'):
            context_lens = torch.tensor([context.shape[1]] * B, dtype=torch.long, device=context.device)
        else:
            context_lens = None
