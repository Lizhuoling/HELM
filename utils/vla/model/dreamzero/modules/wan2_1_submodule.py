...
from utils.vla.model.dreamzero.modules.attention import flash_attention
...
def rope_action_apply_no_polar(
    ...
    num_past_action_per_block: int = 0,
) -> torch.Tensor:
    B, seq_len, n, D = x.shape

    if action_register_length is not None:
        block_width = num_action_per_block + num_state_per_block + num_past_action_per_block
        if action_register_length % block_width != 0:
            raise ValueError(
                f"Action register length {action_register_length} is not divisible by register block width {block_width}."
            )
        chunk_size = action_register_length // block_width
        freqs_1d_action = freqs_action[:chunk_size * num_action_per_block]
        freqs_1d_state = freqs_state[:chunk_size * num_state_per_block]
        freqs_1d_past_action = freqs_action[:chunk_size * num_past_action_per_block]
        freqs = torch.cat([freqs, freqs_1d_action, freqs_1d_state, freqs_1d_past_action], dim=0)
...
def rope_action_apply_polar(
    ...
    num_past_action_per_block: int | None = None,
) -> torch.Tensor:
    B, seq_len, n, _ = x.shape
    ...
    if action_register_length is not None:
        ...
        assert num_past_action_per_block is not None
        ...
        if num_past_action_per_block > 0:
            freqs_1d_past_action = freqs_action[:chunk_size * num_past_action_per_block].view(
                chunk_size * num_past_action_per_block, 1, -1
            )
        else:
            freqs_1d_past_action = freqs_action.new_empty(0, 1, freqs_action.shape[-1])
        freqs = torch.cat([freqs, freqs_1d_action, freqs_1d_state, freqs_1d_past_action], dim=0)
